...
- 🔒 **관리자 패널 제한**: `DISABLE_ADMIN=true` 환경변수 설정 시 관리자 패널 접근을 원천 차단하는 기능을 추가했습니다. 백엔드 API 차단 및 프론트엔드 리다이렉트가 적용됩니다. <!-- id: 26 -->
- 🚨 **Global 401 인터셉터**: 만료된 토큰으로 인해 서버 요청이 실패(401)할 경우, 즉시 로그아웃 처리하고 로그인 페이지로 강제 리다이렉트하는 전역 에러 처리 로직을 추가했습니다. <!-- id: 31 -->

### 변경됨

- 💾 **스토리지 마이그레이션**: 탭이나 창을 닫을 때 토큰을 삭제하여 보안을 강화하기 위해 프론트엔드 토큰 저장소를 `localStorage`에서 `sessionStorage`로 이전했습니다. 또한 로그아웃 시 `localStorage`와 `sessionStorage`를 모두 비우도록 개선하여 잔존 데이터를 완벽히 제거합니다. <!-- id: 10 -->
...
- **프론트엔드**:
  ...
  - `src/routes/+layout.svelte`: Global `window.fetch` 인터셉터 추가 (401 에러 핸들링).
  - `src/lib/components/layout/Sidebar/UserMenu.svelte`: 로그아웃 시 `sessionStorage` 삭제 로직 추가.
  - `src/lib/components/layout/Overlay/AccountPending.svelte`: 로그아웃 시 `sessionStorage` 삭제 로직 추가.
